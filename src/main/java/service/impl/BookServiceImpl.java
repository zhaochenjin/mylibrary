...
public class BookServiceImpl implements BookService{
	...
	public List<Bookinfo> listRecommendationBook(PageBean pageBean, Integer bookId) {
		//设置分页相关属性数据
		PageBean pageBean2=new PageBean(pageBean.getCurrentPage(),5,pageBean.getPageSize());
		pageBean2.setBname(pageBean.getBname());
		pageBean2.setAuthor(pageBean.getAuthor());
		pageBean2.setCname(pageBean.getCname());
		pageBean2.setIndex(pageBean.getIndex());
		pb=pageBean2;
		return bookDao.listRecommendationBook(bookDao.alsoSearch(bookId));
	}
