...
public class BookController {
	...
	@RequestMapping(value = "/library/listRecommendationBook", produces = "text/plain;charset=UTF-8")
	public @ResponseBody String listRecommendationBook(@RequestParam(value = "page", defaultValue = "1") Integer page,
										 @RequestParam(value = "limit", defaultValue = "6") Integer limit,
										 PageBean pageBean,Model model, HttpSession session) {
		pageBean.setCurrentPage(page);
		pageBean.setPageSize(limit);

		AlsoBorrow alsoBorrow = (AlsoBorrow)session.getAttribute("alsoBorrow");
		// 转化为json
		List<Bookinfo> list = bookService.listRecommendationBook(pageBean,alsoBorrow.getBookId());
		PageBean pb=bookService.getPb();
