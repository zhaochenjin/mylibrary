...
<div class="login-main">
    ...
    <form class="layui-form" action="<%=basePath%>library/login.action" method="post">
...
<script type="text/javascript">
    layui.use(['form','layer','jquery'], function () {
        ...
        form.on('submit(login)',function (data) {
            $.ajax({
                url:'<%=basePath%>library/login.action',
                data:data.field,
                dataType:'text',
                type:'post',
                success:function (data) {
                    if (data == '0'){
                        location.href = "<%=basePath%>index.action";//管理员
                    }else if (data == '1') {
                    	 location.href = "<%=basePath%>frontIndex.action";//读者
                    }else if (data == '2'){
                    	layer.msg('请选择正确的权限');
                    }else{
                        layer.msg('登录名或密码错误');
    ...
    function forgetpsw(){
     layer.msg('联系管理员(15369873214)或到图书馆进行重置');
    }
</script>
</body>
</html>